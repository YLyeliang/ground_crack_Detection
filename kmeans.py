...
def Iou(cluster,boxes):
    """
    Calculate the Iou between N boxes and 1 cluster.
    :param:
    box: tuple or array, shifted to the origin (i. e. width and height)
    clusters: numpy array of shape (k,2) where k is the number of clusters
    :return
    numpy array of shape(k,0) whre k is the number of clusters
    """
    x = np.minimum(boxes[:,0],cluster[0])        # shape(r,) element-wise minimum
    y = np.minimum(boxes[:,1],cluster[1])
    if np.count_nonzero(x==0) > 0 or np.count_nonzero(y==0) >0:
        raise ValueError("Box has no area")

    intersection = x*y
    box_area = cluster[0] *cluster[1]
    cluster_area = boxes[:,0] *boxes[:,1]
...
def kmeans(boxes, k ,dist=np.median,seed=1):
    ...
    clusters = boxes[np.random.choice(rows,k,replace=False)]        #(k,2)

    while True:
        # step 1: allocate each item to the closest cluster centers
        for icluster in range(k):       # Calculate cluster-wise distance.
        ...
        # step 2: calculate the cluster cneters as mean (or median) of all the cases in the clusters.
...
def plot_cluster_result(clusters,nearest_clusters,WithinClusterSumDist,wh,k):
    for icluster in np.unique(nearest_clusters):
        ...
        plt.rc('font',size=8)
        plt.plot(wh[pick,0],wh[pick,1],"p",color=c,alpha=0.5,
                 label="cluster = {}, N = {:6.0f}".format(icluster,np.sum(pick)))
        plt.text(clusters[icluster,0],
                 clusters[icluster,1],
                 "c{}".format(icluster),fontsize=20,color="red")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_txt",type=str,default="./data/train.txt")
