...
class Parser(object):
    ...
    def random_crop(self, image, gt_boxes, min_object_covered=0.8, aspect_ratio_range=[0.8, 1.2],
        ...
        begin, size, dist_boxes = tf.image.sample_distorted_bounding_box(tf.shape(image),
    ...
    def preprocess(self, image, gt_boxes):
        image, gt_boxes = utils.resize_image_correct_bbox(image, gt_boxes, self.image_h, self.imgae_w)
    ...
    def preprocess_true_boxes(self, gt_boxes):
        ...
        box_centers = (gt_boxes[:, 0:2] + gt_boxes[:, 2:4]) / 2  # center of box
        box_sizes = gt_boxes[:, 2:4] - gt_boxes[:, 0:2]  # height & width of box
...
class dataset(object):
    ...
    def buildup(self):
        try:
            self.TFRecordDataset = tf.data.TFRecordDataset(self.filenames)
        except:
            raise NotImplementedError("No tfrecords found")
        self.TFRecordDataset = self.TFRecordDataset.map(map_func=self.parser.parser_example, num_parallel_calls=10)
        self.TFRecordDataset = self.TFRecordDataset.repeat() if self.repeat else self.TFRecordDataset
